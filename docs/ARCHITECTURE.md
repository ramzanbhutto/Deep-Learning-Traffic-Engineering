...
|--------|---------------|----------------|
| `graph/` | §2 Network Model | DSA: Graph algorithms, Dijkstra, adjacency structures |
| `traffic/` | §3.1 DM Generation | CN: Traffic models (gravity, bimodal), sparsity |
| `routing/` | §5 Softmin Routing | CN: Multicommodity flow, hop-by-hop forwarding; DSA: Flow computation |
| `baselines/` | §5 Baselines | CN: Classical TE (Prev, Avg_k, Oblivious) |
| `agent/` | §4 RL Formulation, §5 Learning | AI: PPO (continuous control), neural networks; CN: State/action design |
| `train/` | §5 Training Loop | AI: RL training loop, reward shaping, logging |
| `eval/` | §5 Figure 2 | AI/CN: Benchmarking, congestion ratio plots |
...
### 4. `baselines/` — Classical TE Baselines
...
### 5. `agent/` — RL Agent (PPO for Continuous Control)
